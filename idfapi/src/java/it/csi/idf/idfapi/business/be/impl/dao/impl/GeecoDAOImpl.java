...
import it.csi.idf.idfapi.mapper.GeoPlLottoInterventoFeatureMapper;
...
public class GeecoDAOImpl extends AbstractDAO implements GeecoDAO {
	...
	private final GeecoPfaFeatureMapper pfaMapper = new GeecoPfaFeatureMapper();
	private final GeecoVincoloFeatureMapper vincoloMapper = new GeecoVincoloFeatureMapper(); 
	private final GeoPlLottoInterventoFeatureMapper plLottoInterventoMapper = new GeoPlLottoInterventoFeatureMapper(); 
	...
	public List<GeecoAdsFeature> findGeometriaAdsById(String[] idgeoPtAds) throws RecordNotFoundException {
		...
		try {
			...
			return DBUtil.jdbcTemplate.query(sql.toString(), adsMapper);
		} finally {
	...
	public GeecoProfilo findProfiloById(String idProfiloGeeco) throws RecordNotFoundException {
		...
		try {
			...
			sql.append("WHERE id_geeco_profilo = ? ");
			sql.append("AND prof.fk_geeco_prodotto = prod.id_geeco_prodotto ");

			LOGGER.debug("Query per PROFILO GEECO * " + sql + "*");

			// return DBUtil.jdbcTemplate.queryForObject(sql.toString(), profiloMapper,
			// idProfiloGeeco);

			return DBUtil.jdbcTemplate.queryForObject(sql.toString(), new GeecoProfiloMapper(),
					Integer.parseInt(idProfiloGeeco));
		} finally {
	...
	public List<GeecoLayer> findLayerByIdProfilo(String idProfiloGeeco) {
		...
		try {
			...
			sql.append("AND pl.id_geeco_profilo = ?");

			LOGGER.info("Query per PROFILO GEECO * " + sql + "*");
	...
	public GeecoTrsFeature findGeometriaTrsById(String idTrs) throws RecordNotFoundException {
		...
		try {
			...
			sql.append("WHERE idgeo_pl_interv_trasf = ?");

			return DBUtil.jdbcTemplate.queryForObject(sql.toString(), trsMapper, Integer.parseInt(idTrs));
	...
	public List<GeecoTrsFeature> findGeometrieTrsById(String[] idIntervento) throws RecordNotFoundException {
		...
		try {
			...
			LOGGER.info("Query per GEO EVENTO*++++++++++ *" + sql + "*");
			return DBUtil.jdbcTemplate.query(sql.toString(), trsMapper);
	...
	public AreaDiSaggioDatiGeneraliDTO findADSByIdgeoPtAds(Long idgeoPtAds) {
		...
		try {
			StringBuilder sql = new StringBuilder(
					"Select ads.*, ST_X(ST_TRANSFORM(ads.geometria,32632)),ST_Y(ST_TRANSFORM(ads.geometria,32632)), comune.istat_prov istat_prov, ambito.fk_padre as ambitoDiRilevamento ");
			sql.append(
					" FROM idf_geo_pt_area_di_saggio ads left join idf_geo_pl_comune comune on ads.fk_comune= comune.id_comune ");
			...
			list = DBUtil.jdbcTemplate.query(sql.toString(), adsGeneraliMapper, idgeoPtAds);
...
//			sql.append("and pf.idgeo_pl_pfa = pfa.idgeo_pl_pfa "); 
			...
			return DBUtil.jdbcTemplate.query(sql.toString(), eventoMapper, tipo);
		} finally {
	...
	public List<GeecoPfaInterventoFeature> findGeometriePfaInterventoById(String[] idIntervento, String tipo)
			throws RecordNotFoundException {
		LOGGER.debug("[GeecoDAOImpl::findGeometriePfaInterventoById] START");
		try {
			StringBuilder sql = new StringBuilder();
			...
			return DBUtil.jdbcTemplate.query(sql.toString(), interventoMapper, tipo);
		} finally {
			LOGGER.debug("[GeecoDAOImpl::findGeometriePfaInterventoById] END");
		}
	}
...
//			sql.append("where p.idgeo_pl_pfa = a.idgeo_pl_pfa ");
			...
			return DBUtil.jdbcTemplate.query(sql.toString(), new GeecoPfaFeatureMapper());
		} finally {
...
//			sql.append("where p.idgeo_pl_pfa = a.idgeo_pl_pfa ");
			...
			return  DBUtil.jdbcTemplate.query(sql.toString(), pfaMapper);
		} finally {
...
//			sql.append("where p.idgeo_pl_pfa = a.idgeo_pl_pfa ");
	...
	public List<GeecoVincoloFeature> findGeometrieVincoloById(String[] idIntervento) throws RecordNotFoundException {
		...
		try {
			...
			return DBUtil.jdbcTemplate.query(sql.toString(), vincoloMapper);
	...
	public List<GeecoIvnTagliFeature> findGeometrieSelvicolturaliById(String[] idIntervento) throws RecordNotFoundException {
		LOGGER.debug("[GeecoDAOImpl::GeecoIvnTagliFeature] START");
		StringBuilder sql = new StringBuilder();
		try {
			...
			sql.append("json_build_object(");
			sql.append("'type', 'Feature',");
			sql.append("'id', idgeo_pl_lotto_intervento,");
			sql.append("'geometry', ST_AsGeoJSON(geometria)::json,");
			sql.append("'properties', json_build_object(");
			sql.append("'Identificativo', text(idgeo_pl_lotto_intervento),");
			sql.append("'Id intervento', fk_intervento,");
			//sql.append("'Data invio istanza', '',");
			sql.append("'Superficie (ha)', ROUND((st_area(geometria)/10000)::numeric, 4)");
			sql.append(")");
			...
			LOGGER.debug("Query  findGeometrieSelvicolturaliById* " + sql + "*");
			LOGGER.info(sql);
			return DBUtil.jdbcTemplate.query(sql.toString(), plLottoInterventoMapper);
			
		} 
		finally {
			LOGGER.debug("[GeecoDAOImpl::GeecoIvnTagliFeature] Error END "+sql);
		}
	}
