...
@Component
public class WrapperPfaDAOImpl implements WrapperPfaDAO {
	
	private static Logger logger = Logger.getLogger(WrapperPfaDAOImpl.class);
...
//		}
	...
	public void saveLocalizzaLottoNEW(Integer idIntervento, PfaLottoLocalizza body, ConfigUtente loggedConfig,
		...
		intervPfaDAO.updateIntervSelvicolturaleWithLocalita(
				body.getLocalita(), idIntervento);
		skOkSelvDAO.updateFlgSez3(idIntervento);
	}
...
//			logger.info(stimaMassaRetraibile);
				for (GeoPlParticellaForest geoPlParticellaForest : forestParticelle) {
					...
					BigDecimal ripresaIntervento = BigDecimal.valueOf(stimaMassaRetraibile == null ? 0 : stimaMassaRetraibile).multiply(shareInCut);
...
//		saveRicadenzaVincoloIdrogeologico(body.isRicadenzaVincoloIdrogeologico(), idIntervento);
	...
	private IstanzaForest getIstanzaForest(Integer fkConfigUtente, Integer idTipoIstanza, Integer idIntervento) {
		...
		istanzaForest.setNrIstanzaForestale(istanzaForestDAO.getNumberOfInstanceType(idTipoIstanza) + 1);///7777 line comment
	...
	public PlainSecondoSezione getRicadenzePfa(Integer idIntervento) throws ServiceException {
		...
		for(LottoIntervento lotto : listGeomGML) {
			String natura2000ListStr = parkApiServiceHelper.getRicadenzaNatura2000(lotto.getGeometriaGml());
			...
			String areeProtetteListStr = parkApiServiceHelper.getRicadenzaAreeProtette(lotto.getGeometriaGml());
