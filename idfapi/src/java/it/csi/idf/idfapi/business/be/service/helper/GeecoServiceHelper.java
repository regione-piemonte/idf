...
public class GeecoServiceHelper extends AbstractServiceHelper{
	...
	//final String GEECO_AUTH_LEVEL = "liv3";
	...
	final String PROVIDER_ACTALIS = "ACTALIS_EU";
	final String PROVIDER_SPID = "SPID";
	
	
	@Autowired 
	private GeecoDAO geecoDAO;
	
	
	/**
	 * @return the geecoDAO
	 */
	public GeecoDAO getGeecoDAO() {
		return geecoDAO;
	}


	/**
	 * @param geecoDAO the geecoDAO to set
	 */
	public void setGeecoDAO(GeecoDAO geecoDAO) {
		this.geecoDAO = geecoDAO;
	}
	...
	public GeecoServiceHelper(String urlApi, String env) {
		this.urlApi = urlApi;
		this.env = env;
	}

	
	public String getGeecoConfiguration(String idProfiloGeeco, String[] id, String provider) throws ServiceException {

		LOGGER.info("[GeecoServiceHelper::getGeecoConfiguration] START");
		try {
			LOGGER.info("idProfiloGeeco: " + idProfiloGeeco + " - id: " + id);	
			
			Configuration config = this.buildNewConfiguration(idProfiloGeeco, id, provider).build();
	...
	private ConfigurationFactory buildNewConfiguration(String idProfiloGeeco, String[] id, String provider) throws Exception{
		LOGGER.info("[GeecoServiceHelper::buildNewConfiguration] BEGIN");
		try {
			
			String GEECO_AUTH_LEVEL = it.csi.ecogis.geeco_java_client.util.Constants.SHIB_LEVEL_3;
			String GEECO_AUTH_COMMUNITY = it.csi.ecogis.geeco_java_client.util.Constants.SHIB_COMMUNITY_SISP;
			
			if(PROVIDER_SPID.equalsIgnoreCase(provider)){
				GEECO_AUTH_LEVEL = it.csi.ecogis.geeco_java_client.util.Constants.SHIB_LEVEL_2;
				GEECO_AUTH_COMMUNITY = it.csi.ecogis.geeco_java_client.util.Constants.SHIB_COMMUNITY_GASPRP_TERR;
			}
			
			GeecoProfilo profilo = geecoDAO.findProfiloById(idProfiloGeeco);
			...
			//System.out.println("GEECO_AUTH_LEVEL ........................................: " + GEECO_AUTH_LEVEL );
			...
			for (GeecoLayer geecoLayer : layer) {
				...
				// TODO Inizia da qua la parte di configurazione
			...
			//builder.createId(this.createId(id));
			return builder;
		}
		finally {
	...
	private List<GeecoLayer> getLayers(String idProfiloGeeco, String[] id) {
		// Elenco layer
		List<GeecoLayer> layers = geecoDAO.findLayerByIdProfilo(idProfiloGeeco);
		...
		case Constants.GEECO_PROFILO_ADS_INSERIMENTO_MODIFICA:
			if(id!=null && id.length!=0) {
				AreaDiSaggioDatiGeneraliDTO ads = this.geecoDAO.findADSByIdgeoPtAds(ConvertUtil.convertToLong(id[0]));
				if(ads!=null) {
	...
	private Integer createId (String[] id) {		
...
//				result = this.createDefaultValuesADS(geo);
			...
			//FixGP
			case Constants.GEECO_PROFILO_TAGLI_GESTORE_CONSULTAZIONE:
			case Constants.GEECO_PROFILO_TAGLI_CITTADINO_INSERIMENTO_MODIFICA:	
			case Constants.GEECO_PROFILO_TAGLI_DETTAGLIO:
				// Interventi
				result = this.createDefaultValuesTagli(this.buildGeometriePfa(id));
				break; 
	...
	private GeecoAdsFeature buildGeometrieAds(String[] idAds) {
		...
		if(idAds!=null) {
			...
			if(idAds.length==1) {
				...
				List<GeecoAdsFeature> list = geecoDAO.findGeometriaAdsById(idAds);
					return list.get(0);
				}
			else {
...
//		}
	...
	private GeecoIvnTagliFeature buildGeometriePfa(String[] idIntervento) {
		LOGGER.info("[GeecoServiceHelper::buildGeometriePfa] BEGIN");
		GeecoIvnTagliFeature result = new GeecoIvnTagliFeature();
		// result.setDataInserimento(DateUtil.getSqlDataCorrente());findGeometriePfaInterventoById
		if (idIntervento.length == 1) {
			result.setFkIntervento(idIntervento[0]);
		} else {
			result.setFkIntervento("-");
		}
		result.setIdgeoPtLottoIntervento("");
		result.setSuperficie("0");
		LOGGER.info("[GeecoServiceHelper::buildGeometriePfa] END");
		return result;
	}
	...
	//fix GP
	private HashMap<String, Object> createDefaultValuesTagli(GeecoIvnTagliFeature geo){
		LOGGER.info("[GeecoServiceHelper::createDefaultValuesTagli] BEGIN");
		HashMap<String, Object> result = new HashMap<String, Object>();
		
		result.put(Constants.GEECO_TRS_LABEL_IDENTIFICATIVO,geo.getIdgeoPtLottoIntervento());
		result.put(Constants.GEECO_TRS_LABEL_COD_ISTANZA, geo.getFkIntervento());
		//result.put(Constants.GEECO_TRS_LABEL_ID, geo.getIdgeoPtLottoIntervento());
		result.put(Constants.GEECO_TRS_LABEL_SUPERFICIE, geo.getSuperficie());
		
		LOGGER.info("[GeecoServiceHelper::createDefaultValuesTagli] END");
		return result;
	}
	...
	private HashMap<String, Object> createDefaultValuesPfaEvento(GeecoLayer layer, String[] id) {
		...
		if(id!=null && id.length>0) {
			...
			List<GeecoPfaEventoFeature> list =  this.geecoDAO.findGeometriePfaEventoById(id, layer.getFlgTipoLayer());
	...
	private HashMap<String, Object> createDefaultValuesPfaIntervento(GeecoLayer layer, String[] id) {
		...
		case Constants.GEECO_LAYER_PFA_INTERVENTO_POLIGONI:
			...
			//fixGP	
		//case Constants.GEECO_LAYER_TAGLI_INTERVENTO_POLIGONI:		
		...
		case Constants.GEECO_LAYER_PFA_INTERVENTO_PUNTI:
			if (id != null && id.length > 0) {
				...
				List<GeecoPfaInterventoFeature> list = this.geecoDAO.findGeometriePfaInterventoById(id, layer.getFlgTipoLayer());
				if (list != null && !list.isEmpty()) {
	...
	private List<GeoJsonFeature> createGeojsonFeatureList(GeecoProfilo profilo, GeecoLayer layer, String[] id) throws RecordNotFoundException {
		...
		try {
			...
			if(id!=null && id.length>0&&StringUtils.isNotEmpty(id[0])) {
				...
				case Constants.GEECO_PROFILO_ADS_ELENCO_RICERCA:
				case Constants.GEECO_PROFILO_ADS_DETTAGLIO:
					list = geecoDAO.findGeometriaAdsById(id);
				...
				case Constants.GEECO_PROFILO_TAGLI_DETTAGLIO:
				case Constants.GEECO_PROFILO_TAGLI_GESTORE_CONSULTAZIONE:
					result = this.createIvnTagliFeatureS(id); ////7777 vendria null
					break;
				case Constants.GEECO_PROFILO_PFA_CITTADINO_CARTOGRAFIA:
				case Constants.GEECO_PROFILO_PFA_CITTADINO_DETTAGLIO:
				case Constants.GEECO_PROFILO_PFA_CITTADINO_ELENCO:
			...
			if(result==null || result.isEmpty()) {
				//7777
				LOGGER.info("<----- Not exist particelles in table idf_geo_pl_lotto_intervento");
	...
	private List<GeoJsonFeature> createFeatureTrasformazioni(String[] idIntervento) {
		...
		List<GeecoTrsFeature> list = geecoDAO.findGeometrieTrsById(idIntervento);
		if(list!=null && !list.isEmpty()) {
	...
	private List<GeoJsonFeature> createFeatureVincolo(String[] idIntervento) {
		...
		List<GeoJsonFeature> result = new ArrayList<GeoJsonFeature>();
		List<GeecoVincoloFeature> list = geecoDAO.findGeometrieVincoloById(idIntervento);
		if(list!=null && !list.isEmpty()) {
	...
	private List<GeoJsonFeature> createIvnTagliFeatureS(String[] idIntervento) {
		LOGGER.info("[GeecoServiceHelper::createIvnTagliFeatureS] BEGIN");
		List<GeoJsonFeature> result = new ArrayList<GeoJsonFeature>();
		List<GeecoIvnTagliFeature> list = geecoDAO.findGeometrieSelvicolturaliById(idIntervento);
		if(list!=null && !list.isEmpty()) {
			for (GeecoIvnTagliFeature geecoIvnTagliFeature : list) {
				result.add(geecoIvnTagliFeature.getFeature());
			}
		}
		else {
			
		}
		LOGGER.info("[GeecoServiceHelper::createIvnTagliFeatureS] END");
...
//		List<GeoJsonFeature> result = new ArrayList<GeoJsonFeature>();
//		List<GeecoPfaEventoFeature> list = geecoDAO.findGeometriePfaEventoById(idEvento, tipo);
//		if(list!=null && !list.isEmpty()) {
...
//	}
	...
	private List<GeoJsonFeature> createFeaturePfaEvento(String[] idEvento, GeecoLayer layer) {
		...
		if(Constants.GEECO_LAYER_PFA_AREALI.equals(layer.getIdGeecoLayer())) {
			List<GeecoPfaFeature> list = geecoDAO.findGeometriePfaByIdEvento(idEvento);
			...
			List<GeecoPfaEventoFeature> list = geecoDAO.findGeometriePfaEventoById(idEvento, layer.getFlgTipoLayer());
			if (list != null && !list.isEmpty()) {
...
//		List<GeoJsonFeature> result = new ArrayList<GeoJsonFeature>();
//		List<GeecoPfaInterventoFeature> list = geecoDAO.findGeometriePfaInterventoById(idIntervento, tipo);
//		if(list!=null && !list.isEmpty()) {
...
//	}
	...
	private List<GeoJsonFeature> createFeaturePfaIntervento(String[] idIntervento, GeecoLayer layer) {
		...
		List<GeoJsonFeature> result = new ArrayList<GeoJsonFeature>();
		if(Constants.GEECO_LAYER_PFA_AREALI.equals(layer.getIdGeecoLayer())) {
			List<GeecoPfaFeature> list = geecoDAO.findGeometriePfaByIdIntervento(idIntervento);
			...
			List<GeecoPfaInterventoFeature> list = geecoDAO.findGeometriePfaInterventoById(idIntervento, layer.getFlgTipoLayer());
			if (list != null && !list.isEmpty()) {
	...
	private boolean isGeometriaInterventoPresente(String[] idIntervento) {
		LOGGER.info("[GeecoServiceHelper::isGeometriaInterventoPresente] BEGIN");
		int count = this.geecoDAO.countGeometrieIntervento(idIntervento);
		return count>0;
	}
	
	private boolean isGeometriaEventoPresente(String[] idEvento) {
		LOGGER.info("[GeecoServiceHelper::isGeometriaEventoPresente] BEGIN");
		int count = this.geecoDAO.countGeometrieEvento(idEvento);
		return count>0;
	}
...
//		asf.addTextAttributeSchema(new TextSchema(Constants.GEECO_ADS_LABEL_N_ADS,Constants.GEECO_ADS_LABEL_N_ADS, true, false, 30));
	...
	private List<AttributeSchema> getSchemaIstafor() {
		...
		//asf.addTextAttributeSchema(new TextSchema(Constants.GEECO_TRS_LABEL_ID,Constants.GEECO_TRS_LABEL_ID,true, true, true));
	...
	public String getUrlApi() {
		System.out.println("Erro ************************ "+urlApi);	
		return urlApi;
	}
	...
	private List<GeoJsonFeature> createFeaturePfa(String[] idPfa) {
		...
		List<GeoJsonFeature> result = new ArrayList<GeoJsonFeature>();
		List<GeecoPfaFeature> list = geecoDAO.findGeometriePfaById(idPfa);
		if (list != null && !list.isEmpty()) {
			for (GeecoPfaFeature dto : list) {
				if (StringUtils.isNotEmpty(dto.getGeometriaPfa())) {
					result.add(dto.getFeature());
