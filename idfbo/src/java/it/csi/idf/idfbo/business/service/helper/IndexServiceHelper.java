...
import it.csi.idf.idfbo.business.service.helper.factory.IndexDtoFactory;
...
import it.csi.idf.idfbo.util.DateUtils;
...
public class IndexServiceHelper extends AbstractServiceHelper {
	...
	/**
	 * @param ecmEngineWebServiceDelegate the ecmEngineWebServiceDelegate to set
	 */
	...
	public IndexServiceHelper() {
		EcmEngineWebServiceDelegateServiceLocator ecmengineLocator = new EcmEngineWebServiceDelegateServiceLocator();

		try {
			ecmEngineWebServiceDelegate = ecmengineLocator
					.getEcmEngineManagement(new URL(this.getServiceUrl(Constants.URL.URL_INDEX)));
	...
	private Content indexGetContent(String fileName, String typeName, String prefixName) throws ServiceException {
		...
		c.setContentPropertyPrefixedName(Constants.INDEX.INDEX_PREFIX_NAME);
		c.setPrefixedName(prefixName + fileName);
		c.setParentAssocTypePrefixedName(Constants.INDEX.INDEX_PREFIX_CONTAINS);
		c.setTypePrefixedName(typeName);
		c.setMimeType(this.indexGetMimeType(fileName));
		c.setEncoding(Constants.INDEX.INDEX_ENCODING);
	...
	private Content indexGetContentFolder(String folderName) {
		...
		myFolder.setPrefixedName(Constants.INDEX.INDEX_DEFAULT_PREFIX + folderName);
		myFolder.setParentAssocTypePrefixedName(Constants.INDEX.INDEX_PREFIX_CONTAINS);
		myFolder.setModelPrefixedName(Constants.INDEX.INDEX_PREFIX_MODEL);
		myFolder.setTypePrefixedName(Constants.INDEX.INDEX_PREFIX_FOLDER);
	...
	private String indexGetMimeType(String fileName) throws ServiceException {
		...
		if (mime != null && mime.length > 0) {
			...
			return Constants.INDEX.MIMETYPE_DEFAULT;
		}
	}
	...
	private String indexGetRootFolder(String rootFolderName) throws ServiceException {
		...
		try {
			...
			return this.ecmEngineWebServiceDelegate.nodeExists(params,
					indexGetOperationContext(Constants.INDEX.INDEX_USERNAME_ADMIN));
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			uidFolder = this.getEcmEngineWebServiceDelegate()
					.createContent(new Node(uidParent), this.indexGetContentFolder(folderName),
							this.indexGetOperationContext(Constants.INDEX.INDEX_USERNAME_ADMIN))
					.getUid();
		} catch (Exception e) {
	...
	public String indexGetFolder(MetadatiIndexPfa metadati) throws ServiceException {
		...
		try {
			...
			final String annoCorrente = ConvertUtil.convertToString(DateUtils.getCurrentYear());
			final StringBuilder luceneQuery = new StringBuilder();
			// Path /app:company_home/cm:pfa/ cm:<anno>/cm:<idPfa>/

			luceneQuery.append(Constants.INDEX.INDEX_ROOT_PFA).append(Constants.INDEX.INDEX_FOLDER_SUFFIX)
					.append(annoCorrente);
			...
			String uidFolderAnno = this.indexSearchFolder(
					...
					this.indexGetRootFolder(Constants.INDEX.INDEX_ROOT_PFA));

			luceneQuery.append(Constants.INDEX.INDEX_FOLDER_SUFFIX);
...
//				StringBuilder luceneQueryIdIntervento = new StringBuilder(luceneQuery.toString());
	...
	public String indexGetFolder(MetadatiIndexPrimpa metadati) throws ServiceException {
		...
		try {
			...
			luceneQuery.append(Constants.INDEX.INDEX_ROOT_PRIMPA).append(Constants.INDEX.INDEX_FOLDER_SUFFIX)
					.append(metadati.getIdIstanza());

			String uidFolderIdIstanza = this.indexSearchFolder(
					IndexDtoFactory.createSearchParamsForIndexFolder(luceneQuery.toString()), metadati.getIdIstanza(),
					this.indexGetRootFolder(Constants.INDEX.INDEX_ROOT_PRIMPA));

			return uidFolderIdIstanza;
	...
	public String indexGetFolder(MetadatiIndexTrasformazioni metadati) throws ServiceException {
		...
		try {
			...
			luceneQuery.append(Constants.INDEX.INDEX_ROOT_TRASFORMAZIONI).append(Constants.INDEX.INDEX_FOLDER_SUFFIX)
					.append(annoCorrente);

			String uidFolderAnno = this.indexSearchFolder(
					IndexDtoFactory.createSearchParamsForIndexFolder(luceneQuery.toString()), annoCorrente,
					this.indexGetRootFolder(Constants.INDEX.INDEX_ROOT_TRASFORMAZIONI));
	...
	public String indexGetFolder(MetadatiIndexVincolo metadati) throws ServiceException {
		...
		try {
			...
			luceneQuery.append(Constants.INDEX.INDEX_ROOT_VINCOLO).append(Constants.INDEX.INDEX_FOLDER_SUFFIX)
					.append(annoCorrente);

			String uidFolderAnno = this.indexSearchFolder(
					IndexDtoFactory.createSearchParamsForIndexFolder(luceneQuery.toString()), annoCorrente,
					this.indexGetRootFolder(Constants.INDEX.INDEX_ROOT_VINCOLO));
	...
	private String indexGetUidAllegato(SearchParams params) throws ServiceException {
		...
		String uid = null;
		try {
			final OperationContext oc = this.indexGetOperationContext(Constants.INDEX.INDEX_USERNAME_ADMIN);
	...
	public byte[] indexDownloadFilePrimpa(String idAllegato) throws ServiceException {
		...
		return this.indexDownloadFile(Constants.INDEX.INDEX_CM_NAME_PRIMPA, idAllegato);
	}
	...
	public byte[] indexDownloadFilePfa(String idAllegato) throws ServiceException {
		...
		return this.indexDownloadFile(Constants.INDEX.INDEX_CM_NAME_PFA, idAllegato);
	}
	...
	public byte[] indexDownloadFileTrasformazioni(String idAllegato) throws ServiceException {
		...
		return this.indexDownloadFile(Constants.INDEX.INDEX_CM_NAME_TRASFORMAZIONI, idAllegato);
	}

	public byte[] indexDownloadFileVincolo(String idAllegato) throws ServiceException {
		LOGGER.debug("[IndexServiceHelper::indexDownloadFileTrasformazioni] END");
		return this.indexDownloadFile(Constants.INDEX.INDEX_CM_NAME_VINCOLO, idAllegato);
	}

	public byte[] indexDowloadFileByUid(String uidAllegato) throws ServiceException {
		LOGGER.debug("[IndexServiceHelper::indexDowloadFileByUid] BEGIN");
		byte[] result = null;
		try {
			
			LOGGER.info("UID di Index ---> "+uidAllegato);
			...
			LOGGER.error("[IndexServiceHelper::indexDowloadFileByUid] ERROR GENERICO: " + e.getMessage(), e);
			throw new ServiceException("[Errore INDEX indexDowloadFileByUid Exception] " + e.getMessage());
		} finally {
			LOGGER.debug("[IndexServiceHelper::indexDowloadFileByUid] END");
		}
		return result;
	}
	...
	private byte[] indexDownloadFile(String cmName, String idAllegato) throws ServiceException {
		...
		byte[] result = null;
		try {
			final String uidAllegato = this.indexGetUidAllegato(cmName, idAllegato);
			final OperationContext oc = this.indexGetOperationContext(Constants.INDEX.INDEX_USERNAME_ADMIN);
	...
	public String testIndex() throws ServiceException {
		...
		return this.indexGetRootFolder(Constants.INDEX.INDEX_ROOT_PRIMPA);
	}
