...
@Component({
  selector: 'step1-vid',
  templateUrl: './step1-vid.component.html',
  styleUrls: ['./step1-vid.component.css']
})
export class Step1VidComponent implements OnInit, OnDestroy {
  ...
  personaFields = ['codiceFiscale', 'cognome', 'nome', 'comune', 'indirizzo', 'civico', 'cap', 'nrTelefonico', 'eMail'];
  companyFields = ['codiceFiscale', 'partitaIva', 'denominazione', 'comune', 'cap', 'indirizzo', 'civico', 'pec', 'eMail', 'nrTelefonico'];
  ...
  ngOnInit() {
    ...
    if(this.boMode === false) {
      ...
      .subscribe(([resultAdpfor, resultDropdownCodiceFiscale]) => {
        ...
        if(this.editMode) {
          this.vincoloService.getStep1(this.editMode)
          .pipe(takeUntil(this.unsubscribe$))
          .subscribe((result) => {
      ...
      this.vincoloService.getStep1(this.editMode).subscribe(result =>{
        if (result.personaDatiOption === 'RF') {
          this.codiceFiscaleSONOIO[0] = {value: result.codiceFiscale, label: result.codiceFiscale};
        }
        (result.personaDatiOption === 'RF' || result.personaDatiOption === 'PF') ? this.userData = result : this.companyData = result;
    ...
    this.personaDatiOption = personaDatiOption;
      if(personaDatiOption === 'RF') {
        ...
        this.forestaliService.getSoggettiIo().pipe(takeUntil(this.unsubscribe$)).subscribe(res => {
          ...
          if(this.companyData) this.initCompanyForm(this.companyData);
      } else {
        this.restartForm(personaDatiOption);
      }
    //}
  ...
  restartForm(personaDatiOption) {
    switch (personaDatiOption) {
      ...
      case 'PF':
        ...
        this.initPersonalForm(this.editMode ? this.userData : (personaDatiOption === 'PF' ? this.userData : null));
      break;
      case 'RG':
        if(this.editMode){
  ...
  save(nextStep?: boolean) {
    if(this.personaDatiOption === 'RF' || this.personaDatiOption === 'PF') // Persona fisica.
      this.personalOrCompanyForm = this.personaForm;

    if(this.personaDatiOption === 'RG' || this.personaDatiOption === 'PG') // Persona giuridica.
      this.personalOrCompanyForm = this.companyForm;

    if(this.personalOrCompanyForm.valid) {
      this.vincoloService.getAdpfor(sessionStorage.getItem("tipoIstanzaId")).pipe(takeUntil(this.unsubscribe$)).subscribe(res => {
        this.userCompanyData = this.personalOrCompanyForm.getRawValue();
        this.userCompanyData.personaDatiOption = this.personaDatiOption;

        if(this.personaDatiOption == 'PG' && this.cfOwner) this.userCompanyData.ownerCodiceFiscale = this.cfOwner;
        ...
        if(this.editMode) // PUT.
          this.vincoloService.putStep1(this.userCompanyData, this.editMode).pipe(takeUntil(this.unsubscribe$)).subscribe(res => nextStep && this.emitNextStep.emit());
        else { // POST.
          ...
          this.vincoloService.postStep1(this.userCompanyData).pipe(takeUntil(this.unsubscribe$)).subscribe(res => {
            if(!this.editMode) {
