...
export class InserisciNuovaTagliComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (window.location.href.indexOf("return-modifica-tagli") > -1) {
      ...
      console.log(".....> ", this.tagliService.getStep3(this.editMode));
    } else {
      this.setActiveIndex();
    }
  ...
  loadPraticaInviata() {
    this.forestaliService
      ...
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe((response) => {
        console.log(response, this.user, "value");
  ...
  handleValidateWhitAdmin() {
    console.log(this.istanzaInviata, "fnc");
    console.log(this.user, "fnc");
    return true;
  }
