...
import { FoglioModel } from "src/app/shared/models/foglio.model";
import { ParticellaModel } from "src/app/shared/models/particella.model";
...
export class ForestaliService {
  ...
  typeFiltering = {
    ...
    SezioneComune: ["sezione"],
    FoglioComune: ["foglio"],
    ParticellaComune: ["particella"],

    SezioneRicercaComune: ["value"],
    FoglioRicercaComune: ["value"],
    ParticellaRicercaComune: ["value"],
  ...
  findDelega(codiceFiscale: string) {
    return this.httpClient.get<DelegaModel[]>(
      this.config.getBERootUrl(false) + `/deleghe/find?codiceFiscale=${codiceFiscale}`
    );
  }
  ...
  getProvinciaItalia() {
    return this.httpClient.get<ProvinciaModel[]>(
      this.config.getBERootUrl(false) + `/provincia/italia`
    );
  }
 ...
 /*  checkCF(cf:string){
    ...
    formData.append("listcf",cf)
    
    return this.httpClient.post<any>(
      this.config.getBERootUrl(false) +`/soggetti/checkCF`,formData
    )
  } */
  ...
  getSezioni(municipality_code) {
    const reqheaders = {
      "Content-Type": "application/json",
      Authorization: `Bearer 57f8c486-9390-3d5e-82f5-ebeeb6e729ec`,
    };
    return this.httpClient.get<SezioneModel[]>(
      this.config.getBERootUrl(false) + `/sezione/${municipality_code}`
    );
  }

  getFoglio(municipality_code, section_code) {
    const reqheaders = {
      "Content-Type": "application/json",
      Authorization: `Bearer 57f8c486-9390-3d5e-82f5-ebeeb6e729ec`,
    };
    return this.httpClient.get<FoglioModel>(
      this.config.getBERootUrl(false) +
        `/foglio/${municipality_code}/${section_code}`
    );
  }

  getParticella(municipality_code, section_code, sheet_number) {
    const reqheaders = {
      "Content-Type": "application/json",
      Authorization: `Bearer 57f8c486-9390-3d5e-82f5-ebeeb6e729ec`,
    };
    return this.httpClient.get<ParticellaModel>(
      this.config.getBERootUrl(false) +
        `/particella/${municipality_code}/${section_code}/${sheet_number}`
    );
  }
  ...
  getParticellaById(
    ...
    return this.httpClient.get<ParticellaModel>(
      this.config.getBERootUrl(false) +
        `/particella/${municipality_code}/${section_code}/${sheet_number}/${parcel_number}`
    );
  }
}
