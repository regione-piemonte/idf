...
export class PFFormTagliComponent implements OnInit, OnDestroy {
  ...
  filterComune(event) {
    console.log("filterComune", event.query);
    const comune = event.query
    this.forestaliService
      .findComuniByNome(comune)
  ...
  handleFilterResults(opt) {
    const filters = this.codiceFiscaleSelectItem.filter(
      (item) => !!!Number(item.value)
    );
    if (opt === "PF") return filters;
    return this.codiceFiscaleSelectItem;
  }
