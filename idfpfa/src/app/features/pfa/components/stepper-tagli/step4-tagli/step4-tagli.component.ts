...
import { Intervento, IntervPfaFat, TagliStep4 } from "./tagli-step4.model";
...
import { DialogButtons } from "src/app/shared/models/dialog.model";
...
export class Step4TagliComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe((paramMap: ParamMap) => {
      ...
      /*localStorage.setItem(
        ...
        paramMap.get("idIntervento")
      );*/
    });
    this.tagliService.getStepNumber(this.editMode).subscribe((res) => {
      this.isUpdate = (res.nextStep > 3) || (this.fourthFormData.intervPfaFat.tipoInterventoPrincipale.idTipoIntervento != 0);
    ...
    if (this.fourthFormData.intervPfaFat.fkCategoriaIntervento === 1) {
      this.tagliService.getGoverni().subscribe((res) => {
        console.log(res);
        const noGovernopfaId = [6,7]
        this.governiPrincipali = res.filter(el =>  !noGovernopfaId.includes(el.idGoverno));
        this.governiSecondari = res;
        this.changeGovernoPrincipale(
          this.fourthFormData.intervPfaFat.governoPrincipale
        );
        this.changeGovernoSecondario(
          this.fourthFormData.intervPfaFat.governoSecondario
        );
      });
    } else {
      this.tagliService
        .getTipiInterventoCategoria(
          8,
          this.fourthFormData.intervPfaFat.fkCategoriaIntervento
        )
        .subscribe((res) => {
    ...
    this.tagliService.getRicadenze(this.editMode).subscribe((response) => {
      ...
      if (
        ...
        if (this.fourthFormData.specieInteressate) {
          ...
          this.step4Form.patchValue(this.fourthFormData);
          this.specieSelezionateId = this.fourthFormData.specieInteressate.map(el=>el.specie.idSpecie)        
          ...
          this.fourthFormData.specieInteressate.forEach((element, index) => {
            const tempSpecieInteressate: FormGroup = this.fb.group({
              ...
              finalitaTaglio: this.fb.array([], DestinazioneValidator) || true,
            });
            ...
            if (element.specieFinalita) {
              element.specieFinalita.forEach((subElement, subIndex) => {
                ...
                const tempFinalita: FormGroup = this.fb.group({
                  ...
                  mc: ((tempSpecieInteressate.get("mc").value / 100) * perc).toFixed(2),
                  t: ((tempSpecieInteressate.get("t").value / 100) * perc).toFixed(2),
                  q: ((tempSpecieInteressate.get("q").value / 100) * perc).toFixed(2),
  ...
  createFormGroup() {
    return this.fb.group(
      {
        governoPrincipale: [
          this.fourthFormData.intervPfaFat.fkGoverno != 0
            ? this.fourthFormData.intervPfaFat.governoPrincipale
            : null,
          Validators.required,
        ],
        ...
        interventoPrincipale: [
          this.fourthFormData.intervPfaFat.fkTipoIntervento != 0
            ? this.fourthFormData.intervPfaFat.tipoInterventoPrincipale
            : null,
          Validators.required,
        ],
        interventoSecondario: [
          this.fourthFormData
            ? this.fourthFormData.intervPfaFat.tipoInterventoSecondario
            : null,
        ],
        superficiePrincipale: [
          {
            value: this.fourthFormData
              ? this.fourthFormData.intervPfaFat.superficieInt1Ha
              : "",
            disabled: false,
          },
        ...
        superficieSecondaria: [
          {
            value: this.fourthFormData
              ? this.fourthFormData.intervPfaFat.superficieInt2Ha
              : "",
            disabled: false,
          },
        ...
        noteEsbosco: [
          {
            value: this.fourthFormData
              ? this.fourthFormData.intervPfaFat.noteEsbosco
              : "",
            disabled: false,
          },
        ],
        ...
        ramagliaMc: [
          this.fourthFormData.intervPfaFat.volumeRamagliaM3
            ? this.fourthFormData.intervPfaFat.volumeRamagliaM3
            : 0,
        ...
        ramagliaT: [
          this.fourthFormData.intervPfaFat.volumeRamagliaM3
            ? ConverstionVolume.m3ToTon(
                this.fourthFormData.intervPfaFat.volumeRamagliaM3.toString(),
                1
              )
            : 0,
          [Validators.pattern("^[0-9]+(.[0-9]{1,2})?$")],
  ...
  changeGovernoPrincipale(opt?: Governo) {
    if (opt != null) {
      this.tagliService
        .getTipiInterventoGov(3, opt.idGoverno)
        .subscribe((res) => {
          this.tipiInterventoPrincipali = res;
        });
    } else {
      this.tipiInterventoPrincipali = null;
    }
    if(this.step4Form != null){
      this.step4Form.get("interventoPrincipale").setValue(null)
    }
  }
  ...
  changeGovernoSecondario(opt?: Governo) {
    if (opt != null) {
      ...
      this.tipiInterventoSecondari = null;
      this.step4Form.get("interventoSecondario").setValue(null);
      this.step4Form.get("superficieSecondaria").setValue(0);
    }
  }
  ...
  showDialogMsg(msgType:string, msg: string){
    this.dialogService.showDialog(true, msg, msgType, DialogButtons.OK);
  }

  changeSpecie(opt: Specie, idx: number) {
    // (this.step4Form.get('specieInteressate') as FormArray).at(idx).get('mc').setValue(null, { emitEvent: false });
    // (this.step4Form.get('specieInteressate') as FormArray).at(idx).get('t').setValue(null, { emitEvent: false });
    //(this.step4Form.get('specieInteressate') as FormArray).at(idx).get('q').setValue(null, { emitEvent: false });
console.log(opt);
    if(this.specieSelezionateId.includes(opt.idSpecie)){
      this.showDialogMsg('Attenzione', 'Specie già inserita');
      ...
      this.specieSelezionateId.push(opt.idSpecie)
    }
  ...
  deleteSpecie(index) {
    if((this.step4Form.get("specieInteressate") as FormArray).at(index).get("specie").value){
      this.specieSelezionateId = this.specieSelezionateId.filter(el => el != (this.step4Form.get("specieInteressate") as FormArray).at(index).get("specie").value.idSpecie);
    }
  ...
  save(nextStep?: boolean) {
    ...
    selv.fkTipoIntervento2 = 0;
    selv.superficieInt1Ha = this.superficieTotale;
    selv.superficieInt2Ha = thirdFormObject.superficieSecondaria;
    selv.m3Prelevati = +thirdFormObject.totaliMc;
    selv.volumeRamagliaM3 = +thirdFormObject.ramagliaMc;
    selv.stimaMassaRetraibileM3 = +thirdFormObject.totaliMc - thirdFormObject.ramagliaMc;
    selv.noteEsbosco = thirdFormObject.noteEsbosco;
    dataToSend.intervPfaFat = selv;
