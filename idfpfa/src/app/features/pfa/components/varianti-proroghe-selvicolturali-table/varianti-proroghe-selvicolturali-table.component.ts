...
export class VariantiProrogheSelvicolturaliTableComponent
  implements OnInit, OnChanges
{
  ...
  getValue(val: any) {
    if (this.replaceTrueFalse && this.replaceTrueFalse.length == 2) {
      return val == true
        ? this.replaceTrueFalse[0]
        : val == false
        ? this.replaceTrueFalse[1]
        : val;
    }
    return val;
  }
  ...
  getBgColor(car: any, col: any) {
    if (
      ...
      this.getValue(car[col.field]) === "MANCANTE"
    ) {
      return "red";
    }
    if (
      col.field == "varianteProroga" &&
      (this.getValue(car[col.field]) === "VARIANTE" ||
        this.getValue(car[col.field]) === "PROROGA" ||
        this.getValue(car[col.field]) === "SI")
    ) {
      return "yellow";
    }
  }
    ...
    if (this.boModifica) {
      if (item.statoIstanza == "Bozza") {
        this.showDialogError(
          "L’istanza in oggetto è ancora in fase di bozza da parte del richiedente per cui non può ancora essere consultata"
        );
        return;
      }
