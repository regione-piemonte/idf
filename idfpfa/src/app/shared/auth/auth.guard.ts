...
import { AuthService } from "src/app/features/pfa/services/auth.service";
    ...
    if (this.authService.checkPermision()) {
      switch (this.authService.user.ruolo) {
        ...
        case CONST.ROLE_UF_TERRIRORIALE:
          let isUfTerritorialeRoute = CONST.UF_TERRITORIALE_ROUTES.some(
            (route) => {
              return route === this.splitRedirctionURI(url);
            }
          );
        ...
        case CONST.ROLE_CONSULTAZIONE:
          let isConsultazioneRoute = CONST.CONSULTAZIONE_ROUTES.some(
            (route) => {
              return route === this.splitRedirctionURI(url);
            }
          );
        ...
        case CONST.ROLE_SPORTELLO_GESTORE:
          let isSportelloGestRoute = CONST.SPORTELLO_GESTORE_ROUTES.some(
            (route) => {
              return route === next.routeConfig.path;
            }
          );
